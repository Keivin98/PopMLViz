...
function DataUploadModal({samplePCAAdmixDataset, processedPCA, processedAdmix, unprocessedPCA, tsne2d, tsne3d, runPCAir}) {
                    ...
                    <div style={{display: 'flex', flexDirection: 'column', gap: '10px'}}>
                        {
                            sampleDatasets.map((val, index) => {
                                return <Button variant='contained' style={{width: "60%", margin: "auto"}} onClick={() => samplePCAAdmixDataset(0, index)}> <Typography>{val}</Typography></Button>
            ...
            case 'Example: Admixed PCA':
                return (
                    <div style={{display: 'flex', flexDirection: 'column', gap: '10px'}}>
                        {
                            sampleDatasets.map((val, index) => {
                                return <Button variant='contained' style={{width: "60%", margin: "auto"}} onClick={() => samplePCAAdmixDataset(1, index)}> <Typography>{val}</Typography></Button>
                    ...
                    <div>
                        ...
                        <div style={{display: 'flex', justifyContent: 'center'}}>
                            <Button
                                ...
                                onClick={() => {
                                    if (dataProcessed) {processedPCA(files.PCA.processed)}
                                    else {unprocessedPCA(files.PCA.unprocessed, files.PCA.unprocessed.name)}
                                    handleClose()
                                }}>
                                Submit
                            </Button>
    ...
    const renderOptions = () => {
        ...
        switch (dataSelectionStep) {
            case 'initial':
                return (
                    <div style={{display: 'flex', flexDirection: 'column', gap: '10px'}}>
                        <Button variant="outlined" style={buttonStyle} onClick={() => handleDataSelection('own')}>Use Your Own Data</Button>
                        <Button variant="outlined" style={buttonStyle} onClick={() => handleDataSelection('example')}>Use Example Data</Button>
                    </div>
                );
            case 'ownData':
                return (
                    <div style={{display: 'flex', flexDirection: 'column', gap: '10px'}}>
                        <Button variant="outlined" style={buttonStyle} onClick={() => handleProcessingSelection(true)}>Data Processed</Button>
                        <Button variant="outlined" style={buttonStyle} onClick={() => handleProcessingSelection(false)}>Data Not Processed</Button>
                    </div>
                );
            case 'exampleData':
            case 'final':
                ...
                return (
                    <div style={{display: 'flex', flexDirection: 'column', gap: '10px'}}>
                        {options.map(option => (
                            <Button variant="outlined" key={option} style={buttonStyle} onClick={() => handleOptionSelection(option)}>{option}</Button>
                        ))}
                    </div>
                );
            default:
    ...
    const getTitle = () => {
        ...
        switch (dataSelectionStep) {
            case 'initial': return 'Select Data Option';
            case 'ownData': return 'Has the Data Been Processed?';
            case 'exampleData': return 'Use Example Data';
            case 'final': return 'Select Dimensionality Reduction Method';
            default: return '';
        }
    };
    const modalBody = (
        <Box sx={{position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: '60%', bgcolor: 'background.paper', boxShadow: 24, p: 4, borderRadius: 2}}>
            <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px'}}>
                <h2>{getTitle()}</h2>
                {(dataSelectionStep !== 'initial') && <ArrowBackIcon onClick={handleBack} style={{cursor: 'pointer'}} />}
            </div>
            {renderOptions()}
        </Box>
    );
    ...
    return (
        <div>
            <Button variant="outlined" className={classes.customButton} onClick={handleOpen}>Choose Data</Button>
