...
class PCAir extends Component {
	...
	handleFileUpload = (ev) => {
		...
		fetch(`http://${process.env.REACT_APP_DOMAIN}:5000/uploadPCAIR`, {
			method: 'POST',
			body: data,
		}).then((response) => {
			this.setState({
		...
		return (
			<div style={styles.pcair}>
				...
				<div style={styles.uploads}>
					...
					<input type="file" accept=".bed" onChange={this.handleFileUpload} />
				</div>
				<div style={styles.uploads}>
					<label style={{ marginRight: '10%' }}>.bim</label>
					<input type="file" accept=".bim" onChange={this.handleFileUpload} />
				</div>
				<div style={styles.uploads}>
					<label style={{ marginRight: '10%' }}>.fam</label>
					<input type="file" accept=".fam" onChange={this.handleFileUpload} />
				</div>
				<div style={styles.uploads}>
