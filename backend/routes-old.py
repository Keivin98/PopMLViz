...
def runHC():
    request_df = request.get_json()['df']
    num_clusters = int(request.get_json()['num_clusters'])
    if num_clusters < 2:
        num_clusters = 2
    pca_df = pd.json_normalize(request_df)
    
    pca_cols = [x for x in pca_df.columns if 'PC' in x or 'TSNE' in x] or pca_df.columns

    hc = AgglomerativeClustering(n_clusters=num_clusters, affinity='euclidean', linkage='ward').fit(pca_df[pca_cols])

    Z = linkage(pca_df[pca_cols], method='ward')

    # Determine the cutoff distance to form 'num_clusters'
    distance_cutoff = Z[-(num_clusters-1), 2]
    # Plot the dendrogram
    plt.figure(figsize=(12, 12))
    dendrogram(Z, color_threshold=distance_cutoff, truncate_mode='level', p=num_clusters, show_leaf_counts=True)
    plt.axhline(y=distance_cutoff, color='r', linestyle='--')  # Cutoff line
    # plt.xticks([])

    filename = random_string(12)
    plt.savefig('./data/dendrogram/%s.png' % filename, dpi=150)

    len_unique = len(np.unique(hc.labels_))
    return {
        'result': list(map(lambda x: len_unique - int(x) - 1, hc.labels_)), 
...
def runPCAIR():
    ...
    print((bed_name, bim_name, fam_name, gds_name, gds_name, fam_name, result_name))
    if kinship_name == "":
        robjects.r('''
